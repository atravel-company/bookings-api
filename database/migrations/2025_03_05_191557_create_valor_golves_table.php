...
class CreateValorGolvesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('valor_golves', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('pedido_produto_id')->nullable();
            $table->double('valor_golf', 15, 2)->nullable();
            ...
            $table->index(['pedido_produto_id', 'id'], 'valor_golves_pedido_produto_id_IDX');
            $table->index(['total', 'profit', 'valor_golf'], 'valor_golves_total_IDX');
    ...
    {
        Schema::dropIfExists('valor_golves');
    }
}