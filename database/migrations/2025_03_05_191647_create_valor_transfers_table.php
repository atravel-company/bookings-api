...
class CreateValorTransfersTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('valor_transfers', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('pedido_produto_id')->nullable();
            $table->double('valor_transfer', 15, 2)->nullable();
            ...
            $table->index(['id', 'pedido_produto_id'], 'valor_transfers_id_IDX');
            $table->index(['profit', 'total', 'valor_transfer'], 'valor_transfers_profit_IDX');
    ...
    {
        Schema::dropIfExists('valor_transfers');
    }
}