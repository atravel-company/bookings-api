@if (isset($transfers[$key][$key1]))
    ...
    <div class="panel" style="overflow-y: auto;">
        ...
        <div class="w3-row w3-padding" style="overflow-y: scroll;">
            <p>
                {{ Form::open(['id' => 'form_transfers_' . $key . '_' . $key1, 'name' => 'form_transfers_' . $key . '_' . $key1, 'method' => 'GET']) }}
            <div>
            ...
            <input type="hidden" class="key_max" name="key_max_{{ $key1 }}"
                value="{{ count($transfers[$key][$key1]) }}">
            <table class="w3-table w3-striped w3-centered transfer-table">
                ...
                <tr style="background-color: #24AEC9; color: white;">
                    @if (in_array($authUser->email, $users_array))
                        <th class="th-number">
                            <span class="add-transfer" data-key="{{ $key }}" data-key1="{{ $key1 }}"><i
                ...
                @foreach ($transfers[$key][$key1] as $key2 => $transfer)
                    <input type="hidden" name="transfer_id{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                        value="{{ $transfer['id'] }}">
                    <input type="hidden" name="pedido_produto_id{{ $key1 }}"
                        value="{{ $transfer['pedido_produto_id'] }}">
                    <tr>
                        @if (in_array($authUser->email, $users_array))
                            <td>
                                @if ($authUser->id == 2)
                                    @if (isset($transfers[$key]))
                                        <span title="Send transfers to transfergest"
                                            class="remove-transfer sendTransferTransfergest"
                                            data-pedidogeral-id="{{ $pedido->id }}"
                                            data-transfer-id="{{ $transfer['id'] }}"
                                            style="background-color: #fbb040; height: 32px;width: 30px;float: LEFT;border-radius: 100%">
                                            <i class="fa fa-arrow-right"
                                                style="transform:  rotate(-45deg); color:black; font-size: 1.5em "></i>
                                        </span>
                                    @endif
                                @endif

                                <span style="float: right" class="remove-transfer"
                                    onclick="return confirm('Are you sure you want to delete?')? removeRow({{ $transfer['id'] }}, {{ $key }}, $(this), 'transfer'):'';">
                        ...
                        <td>
                            ...
                            <div class="form-group" style="width: 140px;">
                                <div class='input-group date datetimepicker' style="position: relative;">
                                    ...
                                    <input value="{{ Carbon\Carbon::parse($transfer['data'])->format('d/m/Y') }}"
                                        type='text'
                                        name="data{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                        class="th-price form-control ats-border-color checkin_date_transfer"
                                        placeholder="Data">
                        ...
                        <td>
                            ...
                            <div class="form-group" style="width: 140px;">
                                <div class="input-group date timepicker" style="position: relative;">
                                    <div style="position: absolute;"></div>
                                    <input value="{{ $transfer['hora'] }}" type="text"
                                        class="th-price form-control ats-border-color"
                                        name="hora{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                        placeholder="Hora">
                                    <span class="input-group-addon ats-border-color">
                        ...
                        <td>
                            <input value="{{ $transfer['adult'] }}"
                                id="adult{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control w3-block loaddd" type="number"
                                name="adult{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                        </td>
                        <td>
                            <input value="{{ $transfer['children'] }}"
                                id="children{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class=" form-control w3-block loaddd" type="number"
                                name="children{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                        </td>
                        <td>
                            <input value="{{ $transfer['babie'] }}"
                                id="babie{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control w3-block loaddd" type="number"
                                name="babie{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                        </td>
                        <td>
                            <input value="{{ $transfer['flight'] }}"
                                id="flight{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control w3-block loaddd" type="text"
                                name="flight{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                        </td>
                        <td>
                            <input value="{{ $transfer['pickup'] }}"
                                id="pickup{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control w3-block loaddd" type="text"
                                name="pickup{{ $key }}_{{ $key1 }}_{{ $key2 }}">

                        </td>
                        <td>
                            <input value="{{ $transfer['dropoff'] }}"
                                id="dropoff{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control w3-block loaddd" type="text"
                                name="dropoff{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                        </td>

                        @if (in_array($authUser->email, $users_array))
                            <td>
                                <input value="{{ $transfer['company'] }}"
                                    id="company{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                    onchange="" class="form-control w3-block loaddd" type="text"
                                    name="company{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                            </td>
                        @else
                        ...
                        @if (in_array($authUser->email, $users_array))
                            <td>
                                <input value="{{ $transfer['total'] }}"
                                    id="realTransfer{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                    onchange="somaTransfer({{ $key }},{{ $key1 }},{{ $key2 }})"
                                    style="width: 90%;" type="number"
                                    name="realTransfer{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                        ...
                        @if (in_array($authUser->email, $users_array))
                            <td>
                                <input value="{{ $transfer['ats_rate'] }}"
                                    id="atsRateTransfer{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                    onchange="somaTransfer({{ $key }},{{ $key1 }},{{ $key2 }})"
                                    style="width: 90%;" type="number"
                                    name="atsRateTransfer{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                    class="form-control loaddd">
                            </td>
                            <td></td>
                            <td>
                                <span
                                    id="atsProfitTransfer{{ $key }}_{{ $key1 }}_{{ $key2 }}">0.00</span>
                                <span
                                    onclick="enviaTransfersEsp('{{ $transfer['id'] }}','{{ $key }}','{{ $key1 }}','{{ $key2 }}')"
            ...
            <table class="w3-table w3-striped w3-centered transfer-extras-table">
                <tr style="background-color: #24AEC9; color: white;">
                    @if (in_array($authUser->email, $users_array))
                        <th class="th-number">
                            <span class="add-transfer-extras" data-key1="{{ $key1 }}"
                                data-key="{{ $key }}">
                        ...
                        <tr class="extra-tr">
                            ...
                            @if (in_array($authUser->email, $users_array))
                                <td>
                                    <span class="remove-extra"
                                        onclick="return confirm('Are you sure you want to delete?')?removeRow({{ $extra->id }}, {{ $key }}, $(this), 'extra'):'';">
                                        <i class="fa fa-minus-circle fa-2x" aria-hidden="true"></i>
                                    </span>
                                </td>
                            @endif
                                    ...
                                    @foreach ($tipos_extras as $tipo_extra)
                                        @if ($produto[$key][$key1]->pivot->produto_id == $tipo_extra->produto_id)
                                            @if ($tipo_extra->id == $extra->extra_id)
                                                <option value="{{ $tipo_extra->extra_id }}" selected="">
                                                    {{ $tipo_extra->name }}
                                                </option>
                                            @else
                            ...
                            <td>
                                <input value="{{ $extra->amount }}"
                                    id="transfer_extra_amount{{ $key }}_{{ $key1 }}_{{ $id_extra }}"
                                    class="form-control w3-block loaddd" type="number"
                                    onchange="somaExtra('{{ $extra->amount }}',{{ $key }},{{ $key1 }},{{ $id_extra }},'Transfer')"
                                    name="transfer_extra_amount{{ $key }}_{{ $key1 }}_{{ $id_extra }}">
                            </td>

                            @if (in_array($authUser->email, $users_array))
                                <td>
                                    <input value="{{ $extra->rate }}" class="form-control loaddd"
                            ...
                            @if (in_array($authUser->email, $users_array))
                                <td>
                                    <input value="{{ $extra->ats_rate }}" class="form-control loaddd"
                                        id="atsExtraRate{{ $key }}_{{ $key1 }}_{{ $id_extra }}"
                                        onchange="somaExtra('{{ $extra->amount }}',{{ $key }},{{ $key1 }},{{ $id_extra }},'Transfer')"
                                        type="number"
                                        name="atsExtraRate{{ $key }}_{{ $key1 }}_{{ $id_extra }}">
                                </td>
                                <td
                                ...
                                <td>
                                    <span
                                        id="atsExtraProfit{{ $key }}_{{ $key1 }}_{{ $id_extra }}">
                                        0.00
                                    </span>
                                    <span
                                        onclick="enviaExtra('{{ $extra->id }}','{{ $key }}','{{ $key1 }}','{{ $id_extra }}', 3)"
                                        data-changed="false" class="buttonn{{ $key }}"></span>
                                </td>
                            @endif
                        </tr>
                        @php $id_extra++; @endphp
        ...
        <div class="w3-row w3-padding">
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                <section>
                    ...
                    @if (in_array($authUser->email, $users_array))
                        <button style="float: right; margin-top: 4px;" data-pedido_id='{{ $transfer['id'] }}'
                            data-type="transfer" data-toggle="modal" data-target="#modal-edit-remark"
                            aria-hidden="true" class="editremark_button btn btn-default" type="button">Edit
                            Remark
                        </button>
                    @endif
                ...
                <div style="border: 2px solid; height: 177px;">
                    ...
                    <div class="col-md-11">
                        ...
                        <div class="col-xs-7" style="display:{{ $hidden }};">
                            <b>Kick-back</b>
                            @if (in_array($authUser->email, $users_array))
                                <input value="{{ $valorTransfer[$key][$key1]->kick }}" class="loaddd"
                                    id="kickbackTransferInput{{ $key }}_{{ $key1 }}"
                                    onchange="kickbackTransfer({{ $key }},{{ $key1 }})"
                                    type="number" min="0" max="100" name="">
                                <b>%</b>
                            @endif
                        </div>
                        @if (in_array($authUser->email, $users_array))
                        ...
                        @if (in_array($authUser->email, $users_array))
                            <div class="col-xs-7">
                                ...
                                <input value="{{ $valorTransfer[$key][$key1]->markup }}" class="loaddd"
                                    id="markupTransferInput{{ $key }}_{{ $key1 }}"
                                    onchange="markupTransfer({{ $key }},{{ $key1 }})"
                                    type="number" min="0" max="100" name="">
                                <b>%</b>
                            </div>
                        @endif
                        <div class="col-xs-5" style="border-top: 1px solid black;text-align: right;"">
                            ...
                            @endif
                            <span
                                onclick="enviaTransfers('{{ $produto[$key][$key1]->pivot->id }}','{{ $key }}','{{ $key1 }}')"
                                class="buttonn{{ $key }}"></span>
                            <span class="hidden"
                                id="totalProfitTransfer{{ $key }}_{{ $key1 }}"></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row" style="margin-bottom: 10px; margin-top: 10px">
                <div class="col-md-offset-1 col-md-11 ">
                    <span title="Clique to send transfer payments to Transfergest"
                        class="w3-button validarTransfergest pull-right"
                        data-pedidogeral-id="{{ $produto[$key][$key1]->pivot->id }}"
                        style="font-size: 1em; background-color: #aaf1ad">
