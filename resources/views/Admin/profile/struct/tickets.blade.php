@if (isset($tickets[$key][$key1]))
    ...
    <div class="panel" style="overflow-y: auto;">
        ...
        <div class="w3-row w3-padding" style="overflow-y: scroll;">
            {{ Form::open(['id' => 'form_tickets_' . $key . '_' . $key1, 'name' => 'form_tickets_' . $key . '_' . $key1, 'method' => 'GET']) }}
            ...
            <table class="w3-table w3-striped w3-centered">
                ...
                @foreach ($tickets[$key][$key1] as $key2 => $ticket)
                    <input type="hidden" name="ticket_id{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                        value="{{ $ticket['id'] }}">
                    <input type="hidden" name="pedido_produto_id{{ $key1 }}"
                        value="{{ $ticket['pedido_produto_id'] }}">
                    <tr>
                        <td>
                            <!-- Pickup Data -->
                            <div class="form-group" style="width: 140px;">
                                <div class='input-group date datetimepicker' style="position: relative;">
                                    ...
                                    <input value="{{ Carbon\Carbon::parse($ticket['data'])->format('d/m/Y') }}"
                                        type='text' readonly
                                        name="data{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                        class="th-price form-control ats-border-color checkin_date_ticket"
                                        placeholder="Data">
                        ...
                        <td>
                            ...
                            <div class="form-group" style="width: 140px;">
                                <div class="input-group date timepicker" style="position: relative;">
                                    <div style="position: absolute;"></div>
                                    <input value="{{ $ticket['hora'] }}" type="text" readonly
                        ...
                        <td>
                            <input value="{{ $ticket['adult'] }}"
                                id="ticket_adult{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control loaddd" type="number"
                                name="ticket_adult{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                        </td>
                        <td>
                            <input value="{{ $ticket['children'] }}"
                                id="ticket_children{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control loaddd" type="number"
                                name="ticket_children{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                        </td>
                        <td>
                            <input value="{{ $ticket['babie'] }}"
                                id="ticket_babie{{ $key }}_{{ $key1 }}_{{ $key2 }}"
                                class="form-control loaddd" type="number"
                        ...
                        @if (in_array($authUser->email, $users_array))
                            <td>
                                <input value="{{ $ticket['total'] }}"
                                    ...
                                    onchange="somaTicket({{ $key }},{{ $key1 }},{{ $key2 }})"
                                    type="number"
                                    name="realTicket{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                            </td>
                        @else
                            <td>{{ $ticket['total'] }}</td>
                        @endif
                        ...
                        @if (in_array($authUser->email, $users_array))
                            <td>
                                <input value="{{ $ticket['ats_rate'] }}"
                                    ...
                                    onchange="somaTicket({{ $key }},{{ $key1 }},{{ $key2 }})"
                                    type="number"
                                    name="atsRateTicket{{ $key }}_{{ $key1 }}_{{ $key2 }}">
                            </td>
                            {{-- <td></td> --}}
                            <td>
                                <span
                                    id="atsProfitTicket{{ $key }}_{{ $key1 }}_{{ $key2 }}">0.00</span>
                                <span
                                    onclick="enviaTicketsEsp('{{ $ticket['id'] }}','{{ $key }}','{{ $key1 }}','{{ $key2 }}')"
                ...
                <section>
                    ...
                    <div class="remark-box" id="remark-box{{ $ticket['id'] }}">
                        ...
                        @foreach ($tickets[$key][$key1] as $Pedido)
                            @if ($Pedido->remark != null AND $Pedido->remark != "")
                    ...
                    <div class="col-md-11">
                        ...
                        @if (in_array($authUser->email, $users_array))
                            ...
                            <div class="col-xs-7">
                                ...
                                <input value="markupTicket{{ $key }}_{{ $key1 }}" class="loaddd"
                                    id="markupTicketInput{{ $key }}_{{ $key1 }}"
                                    onchange="markupTicket({{ $key }},{{ $key1 }})" type="number"
                                    min="0" max="100" name="">
                                <b>%</b>
                            </div>
                        @endif
...
@endif
<span onclick="enviaTickets('{{ $produto[$key][$key1]->pivot->id }}','{{ $key }}','{{ $key1 }}')"
    class="buttonn{{ $key }}">
