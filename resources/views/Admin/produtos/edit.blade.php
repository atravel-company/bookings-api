<script type="text/javascript" src="{{ URL::asset('js/produtos.js') }}"></script>
...
@if (Auth::check())
		...
		{!! Form::open(['url'=>route('produtos.update',$produto->id), 'method'=>'put']) !!}
		<!-- Nome Form Input -->
    ...
    <div class="form-group">
       ...
       <!-- <select name="categoria_id" class="w3-select w3-border w3-margin-bottom" style="padding: 3px 5px;"> --><br>
        ...
        @if(in_array($categoria->name, $names))
       		<div class="w3-col l4">{{ Form::checkbox('ch[]',$categoria->id,null,['class'=>'w3-checkbox', 'checked']) }}
        {{$categoria->name}}
        </div>
        @else
    ...
    <div class="form-group">
       ...
       <select name="destino_id" class="w3-select w3-border w3-margin-bottom" style="padding: 3px 5px;">
       		...
       		@if(isset($produto->destinos->name) and ($produto->destinos->name == $destino->name) )
  			      <option selected="selected" value="{{$destino->id}}">{{$destino->name}}</option>
  			  @else
    ...
    <div class="form-group">
       ...
       <select name="supplier_id" class="w3-select w3-border w3-margin-bottom" style="padding: 3px 5px;">
         ...
         @if(isset($produto->suppliers->name))
           @if($produto->suppliers->name==$supplier->name)
            <option selected="selected" value="{{$supplier->id}}">{{$supplier->name}} / {{$supplier->social_denomination}}</option>
           @else
	  ...
	  <div class="form-group">
       ...
       @endif
        @if($produto->transfer)
       <div class="w3-col l4">{{ Form::checkbox('transfer',true,null,['class'=>'w3-checkbox', 'checked', 'onchange'=>'UpdateProdutoFormCheckBox(this,"transfer")']) }}
       ...
       @endif
        @if($produto->car)
       <br><br>
        <div class="w3-col l4">{{ Form::checkbox('car',true,null,['class'=>'w3-checkbox', 'checked', 'onchange'=>'UpdateProdutoFormCheckBox(this,"car")']) }}
        {{Form::hidden('car','1')}}
        Rent Car
        </div>
       @else
       ...
       @endif
        ...
        @if($produto->ticket)
       <div class="w3-col l4">{{ Form::checkbox('ticket',true,null,['class'=>'w3-checkbox', 'checked', 'onchange'=>'UpdateProdutoFormCheckBox(this,"ticket")']) }}
    ...
    <div class="form-group">
			...
			{!! Form::text('nome', $produto->nome, ['class'=>'w3-input w3-border', 'style'=>'padding: 3px 5px;']) !!}
	</div>
...
@if($produto->email_type=='no')
       <div class="w3-col l12"><input checked="checked" type="radio" onclick="javascript:yesnoCheck();" name="yesno" id="noCheck" value="no">
       Supplier Reserve Email
        </div>
        <div class="w3-col l12" id="emailsup">
          {!! Form::label('email', 'Email:') !!}
       <select name="emailsup" class="w3-select w3-border w3-margin-bottom" style="padding: 3px 5px;">
        @if($contatos)
          @foreach($contatos as $contato)
              @if($contato->email==$produto->email)
              <option selected="selected" value="{{$contato->email}}">{{$contato->name}} / {{$contato->email}}</option>
              @else
              <option value="{{$contato->email}}">{{$contato->name}} / {{$contato->email}}</option>
              @endif
          @endforeach
        @endif
    </select>
...
<div class="w3-col l12"><input  type="radio" onclick="javascript:yesnoCheck();" name="yesno" id="noCheck" value="no">
       Supplier Reserve Email
        </div>
      ...
      <option value="{{$contato->email}}">{{$contato->name}} / {{$contato->email}}</option>
        ...
        <div class="w3-col l12">
          <input checked="checked" value="yes" type="radio" onclick="yesnoCheck();" name="yesno" id="yesCheck">
          Product Reserve Email
        </div>
...
<div class="form-group w3-center">
			{!! Form::submit('Salvar', ['class'=>'w3-button  w3-blue w3-section w3-padding ']) !!}
		</div>
